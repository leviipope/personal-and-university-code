...
public class Main {
    public static void main(String[] args) {
        ...
        Szak biotech = new Szak("biotech","BT312",4567);
        ...
        hallgatok.add(new Hallgato("VY43O5","Polgári Levente", infoTech));
        ...
        TreeMap<Szak, List<Hallgato>> szakListTreeMap = new TreeMap<>(Comparator.comparing(Szak::getKod));

        for (Hallgato hallgato : hallgatok) {
            Szak szak = hallgato.getSzak();
            if(!szakListTreeMap.containsKey(szak)){
                szakListTreeMap.put(szak, new ArrayList<>());
            }
            szakListTreeMap.get(szak).add(hallgato);
        }

        for(Map.Entry<Szak, List<Hallgato>> entry : szakListTreeMap.entrySet()){
            System.out.println(entry.getKey().getNev());
            for(Hallgato hallgato : entry.getValue()){
